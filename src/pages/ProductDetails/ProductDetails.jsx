    ...
    <section className="bg-[#f7f7f7]">
        ...
        <div className="mt-5 sm:mt-0 sm:pl-8 w-full sm:w-1/2 flex flex-col justify-between ">
          <div className="flex flex-col 2xs:gap-4 lg:gap-8">
            <p className={`font-bold ${fontlg}`}>
              <Reveal>{printerDetails[params.productId]?.title}</Reveal>
            </p>

            <div>
              <Reveal width="w-full">
                {printerDetails[params.productId]?.specSummary?.map(
                  (i, idx) => {
                    return (
          ...
          <Reveal width="w-full" className={"!w-full"}>
            <div className={`flex gap-2 ${mtMd}`}>
              {printerDetails[params.productId]?.type === "printer" && (
                ...
                >
                  <p className={`${fontsm} font-bold`}>Get Drivers</p>
                </Button>
              )}

              <Button
                className="flex-1 rounded-sm sm:rounded-md md:rounded-lg lg:rounded-xl xl:rounded-2xl h-12 xs:h-14 xmd:h-14 sm:h-14 lg:h-16 xl:h-20 shadow-xl "
                title="Enquire Now"
                onClick={() => {
                  navigate("/contact");
                }}
              >
                <p className={`${fontsm} font-bold`}>Enquire Now</p>
              </Button>
            </div>
          </Reveal>
        </div>
      ...
      {printerDetails[params.productId]?.type === "printer" && (
        <div div className="w-[90%] sm:max-w-[1200px] m-auto mb-16">
          ...
          <div>
            {printerDetails?.[params.productId]?.details &&
                ...
                ([key, value], index) => {
                  return (
                    <Reveal width="w-full" className={"!w-full"}>
                      <div
                        key={index}
                        className={`flex flex-col xmd:flex-row justify-between gap-3 `}
                      >
                        <div className="w-full xmd:w-1/3 ">
                          <p
                            className={`${fontmd} font-bold border-b border-black w-full xmd:w-max  leading-[50px]`}
                          >
                        ...
                        <div className="w-full xmd:w-2/3 md:ml-5">
                          {value?.map((items, idx) => {
                            return (
                              <div
                                key={idx}
                                className="flex gap-2 justify-between flex-1 "
                              >
                                <p
                                  className={`w-1/2 ${fontsm} font-medium leading-[50px]`}
                                >
                                  {items?.title}
                                </p>
                                <p
                                  className={`w-1/2 ${fontsm} font-medium leading-[50px] break-words`}
                                >
                                  {items?.desc}
                                </p>
                              </div>
