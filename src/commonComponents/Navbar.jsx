    ...
    <>
      <nav
        className={` w-full py-3 ${
          scrolled ? "bg-white shadow-2xl" : "bg-white bg-opacity-25"
      ...
      >
        <div className="w-full max-w-screen-xl mx-auto flex justify-between items-center px-4">
          <img
            ...
            alt="LogoCWC "
            className="lg:w-28 lg:h-[65px] sm:w-24 sm:h-[50px] xs:w-20 xs:h-[40px] w-18 h-[32px] ms-3"
            loading="lazy"
          />
          <img
          ...
          <ul className="hidden sm:flex gap-3 md:gap-5 lg:gap-6 ml-4 md:ml-6">
            {navList.map((list) => (
              <li
                key={list.key}
                className={`font-bold sm:text-sm md:text-md lg:text-lg xl:text-xl me-4  ${
                  activeTab === list.key
                    ? "border-b-2 border-blue-500 text-blue-500"
                    : "text-black"
                }`}
              >
                <Link to={list.link} onClick={() => setActiveTab(list.key)}>
                  {list.title}
                </Link>
              </li>
            ))}
          ...
          >
            ...
            <ul className="flex flex-col gap-3 mt-8">
              {navList.map((list) => (
                <li
                  key={list.key}
                  className={`flex justify-between items-center font-bold text-xl py-4 border-b ${
                    activeTab === list.key
                      ? "border-b-2 border-blue-500 text-blue-500"
                      : "text-black"
                  }`}
                >
                  <Link
                    to={list.link}
                    className="flex-1"
                    onClick={() => handleNavClick(list.key)}
                  >
                    {list.title}
                  </Link>
                  <img
